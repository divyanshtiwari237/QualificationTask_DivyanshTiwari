...
int main()
{
    ...
    double tf=10.5*pow(10,-9);//final time
    double tStep=0.25*1.78265*pow(10,-11);  // time step;
    ...
    RK2 rk(t0,tf,E,B,tStep);  //initialize the RK2 class with the required fields.     
    rk.rk2(a);  // call the implementing function
    ...
    LeapFrog lp(t0,tf,E,B,tStep); //initialize the LeapFrog class with the required fields.
    lp.leapfrogcall(b); // call the implementing function 
    ...
    writetocsv("Px.csv",A.answerPx,rk.answerPx,lp.answerPx);
    writetocsv("Py.csv",A.answerPy,rk.answerPy,lp.answerPy);
    writetocsv("x.csv",A.answerx,rk.answerx,lp.answerx);
    writetocsv("y.csv",A.answery,rk.answery,lp.answery);
    ...
    for(int i=0;i<n;i++)
    {
        plot.push_back(std::make_pair(lp.answerPx[i],lp.time[i]));    
    }
    Gnuplot gp;
	...
	//gp << "plot '-' using 1:2 with lines'Numerical'\n";
	//gp.send1d(plot);
    //gp.send1d(plot1);*/
